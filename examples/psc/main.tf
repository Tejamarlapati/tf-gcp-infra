...
# -----------------------------------------------------
resource "google_compute_address" "database_private_ip" {
  count        = local.database_sql_instance == null ? 0 : 1
  name         = "db-private-ip-${google_sql_database_instance.database_instance.0.name}"
  address_type = local.database_sql_instance.private_access_config.address_type
  subnetwork   = google_compute_subnetwork.subnets[index(google_compute_subnetwork.subnets.*.name, local.database_sql_instance.subnet_name)].self_link
}

resource "google_compute_forwarding_rule" "database_private_ip_rule" {
  count                 = local.database_sql_instance == null ? 0 : 1
  name                  = "db-private-ip-rule-${google_sql_database_instance.database_instance.0.name}"
  network               = google_compute_network.vpc.self_link
  ip_address            = google_compute_address.database_private_ip.0.id
  load_balancing_scheme = ""
  target                = google_sql_database_instance.database_instance.0.psc_service_attachment_link
}
...
resource "google_compute_instance" "web_server" {
  ...
  metadata_startup_script = templatefile("./startup.sh", {
    ...
    host     = length(google_compute_address.database_private_ip) > 0 ? "${google_compute_address.database_private_ip.0.address}" : ""
